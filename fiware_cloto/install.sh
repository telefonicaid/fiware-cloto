...
if [[ `bash --version | grep 'apple-darwin'` ]]
    then
    #Insert Allowed host into cloto_settings file in Apple systems.
        sed -i "" "s/$matchIp/$matchIp$localIp', '/" $file1;
    else
    #Insert Allowed host into cloto_settings file in Linux systems.
        sed -i "s/$matchIp/$matchIp$localIp', '/" $file1;
fi
...
#cp cloto_settings/fiware-cloto.cfg $config_path
